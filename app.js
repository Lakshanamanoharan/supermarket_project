document.addEventListener('DOMContentLoaded', () => {
    ...
    const inventory = {
        ...
        'grapes':  { stock: 15, unit: 'kg',      price: 150, ta: 'த்ராட்சை',         taUnit: 'கிலோ' }
    };
    ...
    const supermarketDb = {
        'en-IN': {
            greeting: "Hi. What are you ordering today?",
            not_understood: "I'm sorry, I didn't quite catch that. You can ask me to order items or ask about stock availability.",
            responses: [
                { keywords: ['hello', 'hi', 'hey'], reply: "Hello there! What can I help you order today?" },
                { keywords: ['thank you', 'thanks', 'bye', 'goodbye'], reply: "You're very welcome! Have a wonderful shopping experience." }
            ]
        },
        'ta-IN': {
            greeting: "வணக்கம்! இன்று நீங்கள் என்ன ஆர்டர் செய்கிறீர்கள்?",
            not_understood: "மன்னிக்கவும், எனக்கு சரியாக புரியவில்லை. என்ன பொருட்கள் வேண்டும் என்று கேட்கலாம்.",
            responses: [
                { keywords: ['வணக்கம்', 'ஹலோ'], reply: "வணக்கம்! இன்று உங்களுக்கு என்ன பொருட்கள் தேவை?" },
                { keywords: ['நேரம்', 'திறந்திருக்கும்', 'மூடும்', 'எப்போது'], reply: "நோவா பிரெஷ் கடை தினமும் காலை 8 மணி முதல் இரவு 10 மணி வரை திறந்திருக்கும்." },
                { keywords: ['பில்', 'காசாளர்', 'பணம்', 'கவுண்டர்'], reply: "பில் போடும் கவுண்டர்கள் முன் வாசலில் உள்ளன." },
                { keywords: ['நன்றி', 'போய் வருகிறேன்', 'வர்ட்டா', 'பை'], reply: "மிக்க நன்றி! நோவா பிரெஷ்ஷில் உங்கள் ஷாப்பிங் இனிதாக அமையட்டும்." }
            ]
        }
    };
    ...
    async function processUserQuery(query) {
        // We don't necessarily need to stop recognition here if we want it to be continuous,
        // but it's safer to avoid the bot hearing itself.
        // However, for "Real Time", we want it to keep listening but be smart.
        statusText.innerText = currentLanguage.startsWith('en') ? "Thinking..." : "சிந்திக்கிறது...";
        
        if (GROQ_API_KEY) {
            try {
                // Build inventory and cart strings for Prompt Context
                let invStr = Object.entries(inventory).map(([k, v]) => `${k} (${v.stock}${v.unit})`).join(', ');
                let cartStr = Object.keys(cart).length === 0 ? 'Empty' : Object.entries(cart).map(([k, v]) => `${k}: ${v.qty}`).join(', ');

                const systemPromptEn = `You are an ultra-fast grocery ordering assistant. 
INVENTORY: ${invStr}. (NEVER sell more than inventory). 
CURRENT CART: ${cartStr}.
Keep conversation VERY short (<15 words). 
RULE 1: If user requests more than stock, tell them exactly what we have and ask: "Is that okay?"
RULE 2: If user agrees/says ok to an amount, add it to cart using: [ADD: item, quantity]
RULE 3: If user wants to REMOVE, CANCEL, or SUBTRACT an item, use: [REMOVE: item, quantity]. If no quantity is mentioned, remove ALL: [REMOVE: item].
RULE 4: Prioritize [REMOVE] over [ADD] if the user says "remove", "cancel", or "don't want".
Example: Removed 1kg of tomato. [REMOVE: tomato, 1kg]`;

                const systemPromptTa = `நீங்கள் Nova Fresh AI.
கையிருப்பு: ${invStr}. (கையிருப்புக்கு மேல் விற்க வேண்டாம்).
தற்போதைய Cart: ${cartStr}.
பயனர் "நீக்கு" அல்லது "வேண்டாம்" என்று சொன்னால்: [REMOVE: பொருள், அளவு] பயன்படுத்தவும்.
சுருக்கமாக பேசவும். கண்டிப்பாக தமிழில் மட்டுமே பதிலளிக்கவும். கட்டளைகள்: [ADD: பொருள், அளவு], [REMOVE: பொருள்].`;
